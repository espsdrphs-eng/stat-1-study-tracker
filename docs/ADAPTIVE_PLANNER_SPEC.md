...
- D90・D60・D30診断は純粋関数として実行し、DB、todayPlanSnapshotを変更しない。unknown素材を直接答案化せず、
  将来配分の容量診断では「チェックポイント前に素材選択確認を完了」という仮定を画面へ明示する。

## 参照パック

- 正規化入力は `stat1_exam_reference_pack_v1`。PDFや外部Webを実行時に再解析しません。
- ZIP全体のSHA-256、manifest記載の各ファイルbytes/SHA-256、schema、件数を検証します。
- 2016〜2019年、2021〜2025年の45大問を検証済みcoreとして登録します。2020年は問題を作りません。
- 2016〜2018年の15件は `STAT1-PAST-EXAM-2016-2018-v1` の確定要約を既存concept registryへ照合し、
  初期露出 `unknown`、模試保護なしで登録します。露出確認前に実答案へ直接配置しません。
...
- 2024・2025年は模試保護を初期値とし、残り61日以上では `unseen / unknown / prompt_scanned` の
  どの状態でも initial answer、full、timed、scan5 の最終候補から除外します。
...
- 日次は得点形成1〜2件、局所補修最大1件、維持・選択0〜1件。候補が十分な日は60〜90分の
  有効なcore planを作り、設定学習時間を超えません。
...
- `◎` の有無は卒業条件にしません。`delayed_retrieval` で、参照0、ヒントなし、success、
  `target_issue_resolved`、`minimum_pass_condition_met`、全graded partが `none + resolved`、
  K/W/N/Cなし、未解決carryoverなしを同時に満たした場合を客観成功とします。
- `retrieval_check` の客観成功後は同じproblem・同じpurposeのcheckを自動生成しません。
- `error_repair` の成功後に全体統合が必要なら別purposeの `integration_check`、検証済み転移先が
  必要なら別問題の候補として扱います。同じ問題をtransferと偽装しません。
- 参照使用、ヒント使用、未解決finding、same-session成功では卒業せず、従来の必要な確認を残します。
- 過去のmark、Attempt、Reviewは変更しません。

## 診断の正本

- 通常画面、14日/30日診断、診断パックのplanner sourceは `adaptive` です。
- 旧planner比較はロールバック用の開発者診断に限定し、正式件数・配分へ混在させません。
