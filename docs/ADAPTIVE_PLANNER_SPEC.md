...
- 未使用容量は `targetMinutes - completedMinutes - activeRemainingMinutes` とし、shadowの得点形成、試験得点課題、第5・7章維持、有効な先送り候補の順で最大3件を追加候補として提示する。
...
- 新concept指標はshadow専用で、旧K/W/N/C集計と同じランキングへ混ぜない。強い独立失敗が0件なら、暫定失敗率が高くても「要診断」であり「確認済み弱点」ではない。
...
- D90・D60・D30診断は純粋関数として実行し、DB、todayPlanSnapshot、shadow観察開始日は変更しない。
...
- 年度の候補順はshadow plan、露出状態、残り日数、模試保護から導出します。
