...
test("復習採点プロンプトは前回課題と今回の改善を比較する",()=>{
  ...
  assert.match(prompt,/参照を閉じた後に再現したか：はい/);
  ...
  assert.match(prompt,/前回ミスを見ただけで自動的にpartialへ下げない/);
  assert.match(prompt,/actual_reference_level が allowed_reference_level を超えた場合だけ/);
  ...
  assert.match(prompt,/next_actionには日付や復習間隔を書かない/);
  assert.match(prompt,/参照を閉じた後に再現していない場合はsuccessにしない/);
});
