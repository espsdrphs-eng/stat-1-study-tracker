...
test("ノート補修には骨格シートと専用の記入欄を出す",()=>{
  ...
  assert.deepEqual(template.fields.map(field=>field.label),[
    "修正ルール1行","不足していた説明","方針・入口","出発式","今見る量",
    "先に確認すること","使う道具","解答の流れ","最後に示すこと","ここから先は計算"
  ]);
});
...
test("復習種別から表示シートのモードを補完する",()=>{
  ...
  assert.equal(reviewMode({review_type:"careless_check"}),"check");
