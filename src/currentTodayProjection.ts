import type {Attempt,ProblemAlias,Review,Task,TodayPlanSnapshot} from "./types.ts";
...
export function deriveCurrentTodayProjection(args:{
  snapshot:TodayPlanSnapshot;generatedTasks:Task[];attempts:Attempt[];reviews:Review[];today:string;
  ...
  return deriveCurrentTodayState({tasks,attempts:args.attempts,snapshot:args.snapshot,aliases:args.aliases,
