...
function actionFor(error:string,hasS:boolean){
  ...
  if(error==="N") return {action:"弱点ノートの修正ルールを確認し、途中式を省略せず骨格を再現する",mode:"skeleton",minutes:15,load:.5};
...
export function analyzeWeaknesses(
  ...
  for(const [theme,rows] of groups){
    ...
    for(const row of rows){
      ...
      errors.forEach(error=>{counts[error]++;score+=errorWeight[error]});
      if(row.score_numeric!=null) score+=row.score_numeric<70?2:row.score_numeric<80?1:0;
      if(overdueProblems.has(row.problem_id)) score+=2;
    }
    const recurrence=Math.max(0,failed.length-1);
    score+=recurrence*2;
    const latest=[...rows].sort((a,b)=>b.date.localeCompare(a.date)||b.id-a.id)[0];
    if(["◎","○"].includes(latest.mark)&&errorsFor(latest).length===0) score=Math.max(0,score-2);
    ...
    const evidence=[
      `${rows.length}回の記録中、要対策 ${failed.length}回`,
    ...
    insights.push({
      theme,score:Math.round(score),level:score>=10?"重点":score>=5?"注意":"観察",confidence,
